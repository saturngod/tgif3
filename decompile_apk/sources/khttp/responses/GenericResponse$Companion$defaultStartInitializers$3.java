...
final class GenericResponse$Companion$defaultStartInitializers$3 extends Lambda implements Function2<GenericResponse, HttpURLConnection, Unit> {
    ...
    public final void invoke(@NotNull GenericResponse genericResponse, @NotNull HttpURLConnection httpURLConnection) {
        ...
        Map cookies = genericResponse.getRequest().getCookies();
        if (cookies != null) {
            httpURLConnection.setRequestProperty("Cookie", new CookieJar(MapsKt__MapsKt.plus(cookies, (Map) genericResponse._cookies)).toString());
        }
    }
}
